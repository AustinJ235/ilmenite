use crate::ImtGlyphBitmap;
use crate::ImtShaderVert;
use crate::ImtError;
use crate::ImtShapedGlyph;
use crate::ImtParser;
...
impl ImtRaster {
	pub fn new(device: Arc<Device>, queue: Arc<Queue>, opts: ImtRasterOps) -> Result<Self, ImtError> {
	...
	pub fn raster_shaped_glyphs(
		...
		let bitmap_cache = unsafe { &mut *(self as *mut Self)}.rastered_glyphs
		...
		for shaped in shaped_glyphs {
			...
			if bitmap_cache.get(&index).is_none() {
				let mut bitmap = ImtGlyphBitmap::new(
					parser,
					shaped.parsed.clone(),
					text_height
				);
